...
class flickerBox { 
	constructor(parentElem, i, freq, txt, opts){
		...
		this.switchTimes = [];
		if (this.flickerText==true) this.squarediv.style.backgroundColor = "gray";
		...
		if (this.flickerText==true) {
			...
			this.squarediv.style.backgroundColor = 'gray';
		} else {
			this.squarediv.style.color = 'gray';
...
class SSVEP{
	constructor(parentElem){
		...
		this.stimulationCount = 1;
		this.stimulationCurrentCount = 0;
		this.stimulationTime = 5000;
		this.breakTime = 2000
		this.parentElem = parentElem;
	...
	createSetupUI(){
		...
		<tr><td></td><td>Columns</td><td><input type="text" name="columns" value="5" placeholder="Columns"></td><td></td></tr>
		<tr><td></td>
		...
		for (let i=0; i<this.boxesCount; i++) {
			$(this.setupUIElem).find("div.setupTable").find("tbody").append('<tr><td>'+(i+1)+'</td><td><input type="text" name="freq" value="'+(13+i*0.4)+'"></td><td><input type="text" name="text" value="'+String.fromCharCode("A".charCodeAt(0)+i)+'"></td><td><div class="removeBtn"></div></td></tr>');
		}
	...
	startRun() {
		...
		const e = document.querySelector('div.setupPage tfoot input[name="columns"]');
		if (e){ options.cols = parseInt( e.value ); }
		window.eel.start()(function(response) {
			console.log("Response from Python:", response);
		});
		// this.startStimulation(boxes, options);
		this.controllerStimulation(boxes, options)
		return false;
	}

	controllerStimulation(boxes, options){
		this.startStimulation(boxes, options)
		this.stimulationCurrentCount += 1
		let timmer = setTimeout(() => {
			this.stopStimulation()
			if (this.stimulationCurrentCount > this.stimulationCount) {
				window.eel.stop()(function(response) {
					console.log("Response from Python:", response);
				});
				clearTimeout(timmer)
				setTimeout(() => {
		...
		}, this.stimulationTime);

	}

	startStimulation(boxInfos, options) {
		window.eel.trigger(this.stimulationCurrentCount)(function(response) {
			console.log("Response from Python:", response);
		});
